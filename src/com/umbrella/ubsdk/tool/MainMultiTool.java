...
//            exeCmd("java -jar -Xms512m -Xmx512m apktool.jar if framework-res.apk", apktool);
        ...
        for (GameChannel gameChannel : gameChannelList)
        {
            ...
            String sdkPath = CONFIG_PATH + File.separator + "sdk" + File.separator + gameChannel.getFolder();
            ...
            // 清空temp目录
            System.out.println("清空temp目录,每一个渠道的资源都是不一样");
...
//            mutiSmali();
            ...
            String finalApkPath = outPath + File.separator + gameItem.getFolder() + "_" + gameChannel.getId() + "_" + dateTime + ".apk";
        ...
        System.out.println("打包程序执行完成,所有的包已出>>>>>>>>>>>>>OK！");
    ...
    private static void mutiSmali()
    {
        ...
        try
        {
        	...
        	String smalipyPath = "D:\\VasSDK\\VasSDK\\FrameworkSDK\\VASSDKTool\\apktool\\smali.py";
//            Process pr = Runtime.getRuntime().exec("D:\\Python\\Python362\\python.exe " + smalipyPath);
            Process pr = Runtime.getRuntime().exec("D:\\Python\\Python27\\python.exe " + smalipyPath);
    ...
    private static void executeOperation(String sdkPath, String tempPath, List<Operation> operations) throws Exception
    {
        for (Operation operation : operations)
        {
            ...
            else if("copylib".equals(operation.getType()))
            {
...
//                    newPackageVal = oldPackageVal + suffix;//直接拼，不截取
            ...
            throws Exception
    {
        ...
        {
        	System.out.println("VAS_GAMEID = " + game.getGameId());
...
//        {
            ...
            addParamElement(rootElement, "MAIN_ACTIVITY_NAME", mainActivityName);//这里动态添加，所以game.xml中的配置没有用到。
//        }
