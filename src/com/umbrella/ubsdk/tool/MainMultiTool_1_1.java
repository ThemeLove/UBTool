...
1、games.xml 文件都只需要放到games文件夹下面就可以，无需单独放到各个游戏文件下面,chanel.xml 在每个游戏包下面
...
(注意参数只包含：其他参数如有需要看情况
    ...
    gameId; //游戏Id=================================VAS_GAMEID（必须），已经弃用，可以不配置了
...
4、sdk下面的config.xml 配置lib拷贝的type类型需要更改为copylib,需要继续优化，1-1之后不需要改动了

5、多种游戏的同事打包跟多渠道的一样，暂时不支持输入错误重新输入的情况

6、games下面的游戏包apk不需要重新修改名了，代码中自动获取apk名字；注：一个文件夹下面只可以有一个apk包，这里没有做多个apk的区分

7、apktool 版本自动切换，在game.xml中配置版本号就可以了，如 2-3-0

8、添加分包的开关，OPEN_MULTSMALI，true或false

9、特殊情况，拷贝pptv资源到classes2中，开关Copy_To_Classes2，目前只有这一种，个别包需要 用到

10、一个channel中存在一个渠道多个游戏的情况处理，添加folder属性：这个在1-2版本中比较常用，如：
		<channel id="tianyuyou-lzdzj" folder="tianyuyou" name="天宇游SDK-龙之岛战纪" suffix=".tyy" splash="true" icon="rb"  >

11、1-1版本与1-2版本的区别，channels获取的位置不同，1-1版本取得是游戏名下面的，1-2版本取得是games下面的全局的channels

12、1-2版本中添加ONLINE_OUT_PPTV  快速打pptv懒人式打包开关，默认是打开的
 */

public class MainMultiTool_1_1
{
...
//        System.setOut(printStream);
      ...
      for(Game gameItem : gameList){//work一种游戏只能使用一次，下一个游戏打包之前要删除
           ...
           */
          String gameChannelXmlPath = GAMES_PATH + File.separator+gameItem.getFolder()+ File.separator + "channel.xml";
